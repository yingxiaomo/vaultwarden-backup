...
services:
  vaultwarden-backup:
    image: ghcr.io/yingxiaomo/vaultwarden-backup:dev
...
所有配置项均可通过 Web 面板的配置页面直接修改，无需手动编辑环境变量或配置文件。

### 初次使用

1. 启动容器后访问 `http://宿主机IP:9876`
2. 按照引导完成管理员账号和基础设置
3. 登录后在配置页面调整备份策略、通知、云存储等选项

### Web 面板功能

| 功能 | 说明 |
|------|------|
| **仪表盘** | 查看备份历史、执行手动备份、恢复指定备份 |
| **配置** | 修改数据库、定时任务、云存储、通知等所有设置 |

配置数据保存在挂载卷 `./config:/app/config` 中，容器重启后自动生效。
