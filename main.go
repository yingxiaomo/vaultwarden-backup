...
func logAppend(cfg Config, format string, args ...interface{}) {
	msg := fmt.Sprintf(format, args...)
	t := time.Now().Format("2006-01-02 15:04:05")
	line := fmt.Sprintf("[%s] %s\n", t, msg)
	fmt.Print(line)
	logFile := "/app/config/backup.log"
...
func getFreeSpaceMB(dir string) (int, error) {
	...
	if len(fields) < 4 {
		return 0, fmt.Errorf("unexpected df line: %s", lastLine)
	}
	availStr := fields[len(fields)-2]
	re := regexp.MustCompile(`^(\d+)`)
	m := re.FindStringSubmatch(availStr)
	if len(m) < 2 {
		return 0, fmt.Errorf("cannot parse available space: %s", availStr)
	}
...
func runBackup(cfg Config) int {
	...
	logf(cfg, "%s...", T(cfg.Lang, "cleanup_temp"))
...
func main() {
	...
	// 默认模式：后台调度（常驻容器，按 CRON_SCHEDULE 定时备份）
	// 行为与旧版 Python 容器的 crond 一致
	log.Println("启动后台调度模式，按 CRON_SCHEDULE 定时备份...")
	log.Println("提示：使用 --once 执行一次性备份，使用 --web 启动 Web 面板")
	startCronScheduler()
	select {}
}
